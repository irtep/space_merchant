...
const PlayScreenV2: React.FC = () => {
    ...
    useEffect(() => {
        ...
        characters.forEach(c => {
            ctx.beginPath();
            ctx.arc(c.x * CELL_SIZE + CELL_SIZE / 2, c.y * CELL_SIZE + CELL_SIZE / 2, CELL_SIZE / 2 - 2, 0, Math.PI * 2);
        ...
        <div className="w-full h-full flex flex-col">
            ...
            <div className="flex flex-1">
                ...
                <div className="w-2/5 p-4 bg-blue-50 flex flex-col gap-2">
                    ...
                    {characters.map(c => (
                        <button key={c.id} onClick={() => setSelectedId(c.id)} className={`p-2 rounded border ${selectedId === c.id ? "bg-yellow-300" : "bg-white"}`}>
